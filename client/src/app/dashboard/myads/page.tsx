        ...
        <div className="w-full relative flex flex-row items-center justify-center flex-wrap content-center py-9 px-0 box-border gap-6">
          ...
          {data.ads.map((item: any) => (
            <Itemcard
              ...
              price={item.price} location={item.address} itemImage={item.ImageOne && item.ImageOne.url} daysAdded="2" />
          ))}

        </div>
      ) : (
        <div className='text-black flex items-end justify-center font-medium'>You are not authorized! Please Login</div>
      )}
